...
# and `pip install mnist_trainer` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "mnist_trainer", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n mnist_trainer -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "mnist_trainer", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "mnist_trainer", "YOUR_CONSOLE_SCRIPT"]
